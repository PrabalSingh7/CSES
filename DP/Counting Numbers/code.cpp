...
ll solve (string num, int n, int prev, bool leading_zeros, bool tight)	{
	if(n==0)
		return 1;
    if(prev!=-1 && dp[n][prev][leading_zeros][tight] != -1)
	...
	for(int dig=lb; dig<=ub; dig++)	{
		if(dig==prev && leading_zeros==false)
			continue;
		retVal += solve(num, n-1, dig, (leading_zeros & (dig==0)), (tight & (dig==ub)) );
	}

	return dp[n][prev][leading_zeros][tight] = retVal;
} 
...
void solve(int abcdefg )	{      
	...
	ll val1 = solve(B, B.length(), -1, 1, 1); 
	memset(dp, -1 ,sizeof dp);
	ll val2 = solve(A, A.length(), -1, 1, 1);
