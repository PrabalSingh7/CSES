...
void solve(int abcdefg )	{  
	...
	ll nod = 1;
	for(int i=0; i<n; i++)	
		nod = (nod * (power[i]+1)) % mod;

	ll sod = 1;
	for(int i=0; i<n; i++)	
		sod = (sod * geometricSum(prime[i], power[i])) % mod;
	...
	if(oddExp)	{
		...
		for(int i=0; i<n; i++)	{
			if(i == posOfOddExp)	
				outterExp = (outterExp * (power[i]+1)/2) % (mod-1);
			else 
				outterExp = (outterExp * (power[i]+1)) % (mod-1);
		}
		for(int i=0;i<n; i++)
			pod = ( pod * modBinExp(prime[i], (power[i]*outterExp)%(mod-1) ) ) % mod;
	}
	else {
		ll outterExp = 1;
		for(int i=0; i<n; i++)
			outterExp = (outterExp * (power[i]+1)) % (mod-1);
		for(int i=0; i<n; i++)
			pod = ( pod * modBinExp(prime[i], ((power[i]/2)*outterExp)%(mod-1)))%mod;
	}
...
//  FOR REFERNCE TO FORMULAS USED ABOVE : CHECK YOUTUBE VIDEO OF DIVISOR ANAYLSIS BY ARS LONGA VITA BREVIS
