...
void solve(int abcdefg )	{   
	...
	for(; i<n; i++)	{
	   // for(auto it : os)   cout << it <<" ";   cout<<"\n";
		if(k%2==1)	{
			int midIdx = k/2;
			int median = *os.find_by_order(midIdx);
			cout << median << " " ;
			int to_del = arr[i-k];
			myerase(os, to_del);
		}
			...
			int medIdx1 = (k/2) - 1;
			int medIdx2 = k/2;
			int median1 = *os.find_by_order(medIdx1);
			int median2 = *os.find_by_order(medIdx2);
			int median = min(median1,median2);
			cout << median << " " ;
			int to_del = arr[i-k];
			myerase(os, to_del);
		}
