...
pair<bool,vector<int>> twoSum(vector<pair<int,int>> &arr, int findSum, int idxA, int n) 	{
	int l= idxA==0 ? 1 : 0, h=idxA==n-1 ? n-2 : n-1;

	vector<int> res ;
	res.push_back(arr[idxA].second);
...
void solve(int abcdefg )	{   
    ...
    // for(auto it : arr)  cout << it.first<<" @ "<<it.second<<"\n";  cout<<"\n";
  	...
  	for(int i=0; i<n; i++)	{
  		int ts = target - arr[i].first;
  		if(ts <= 0)	break;

  		auto it = twoSum(arr,ts,i,n);
  		// cout << i << " " << ts << "\n";
