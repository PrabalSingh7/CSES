...
void fun(int idx, vector<ull> &util, int reqLen, ull &neg, ull &pos, vector<ull> &arr, ull n, unordered_map<ull,int> &mp)	{ 
    int sn = util.size();
    
	if(idx>=arr.size())	{
		if(sn==reqLen)	{
			ull val = 1; 
			for(auto it : util)	val *= it;

// 			for(auto it : util)	cout<<it<<" ";	cout<<"\n";
// 			cout<<val<<"\n";
...
// 			cout<<val<<"\n";
	...
	//take 
	util.push_back(arr[idx]);
	fun(idx+1, util, reqLen, neg, pos, arr, n, mp );
