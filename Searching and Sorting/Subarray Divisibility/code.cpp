...
// 	for(auto it : pmod)  cout<<it<<" ";  cout<<'\n';
    
    for(int i=0; i<n; i++)  {
        if(mp.find(pmod[i])!=mp.end())	{
			res += mp[pmod[i]].size();
		}
		
		
// 		if(pmod[i] < 0) cout << n + pmod[i] <<" ";
// 		if(pmod[i] > 0) cout << pmod[i] - n <<" ";
		
		if(pmod[i]<0 && mp.find(n+pmod[i])!=mp.end())   {
		    res += mp[n+pmod[i]].size();
		}
		if(pmod[i]>0 && mp.find(pmod[i]-n)!=mp.end())   {
		    res += mp[pmod[i]-n].size();
		}
		
		if(pmod[i]==0)	{
			res += 1;
		}
		mp[pmod[i]].push_back(i);
    }
