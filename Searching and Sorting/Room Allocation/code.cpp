...
// 	for(auto it : arr)   cout<<it.first<<" "<<it.second<<"\n";  cout<<"\n";
	...
	for(int i=0; i<n; i++)	{ 
	    int temp_entry = arr[i].first;
	    
	    op.insert( temp_entry );
	   // for(auto it : op)   cout<<it<<" ";  cout<<"\n"; 
	    
	    int idx = op.order_of_key( temp_entry ); 
	    int curr = i + 1 - idx;
	    mxi = max(mxi, curr);
	    vec[i] = curr;
	    
	   // op.erase( temp_entry );
	    myerase(op,temp_entry);
	   // for(auto it : op)   cout<<it<<" ";  cout<<"\n";
	    
	    op.insert( arr[i].second.first );
	   // for(auto it : op)   cout<<it<<" ";  cout<<"\n";
...
// 	for(auto it : vec)  cout<<it<<" ";  cout<<"\n";
	...
	for(;i<mxi; i++) { 
	    pq.push({arr[i].second.first,{ vi++, arr[i].second.second}});
	    res[arr[i].second.second] = vi - 1;
	}
	
	for(; i<n; i++)  { 
	   auto it = pq.top();
	   pq.pop();
	   int vac = it.second.first; 
	   pq.push({arr[i].second.first,{vac,arr[i].second.second}});
	   res[arr[i].second.second] = vac;
	   //cout<<vac<<" ";
	}
